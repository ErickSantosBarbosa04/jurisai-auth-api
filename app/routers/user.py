...
def get_me(current_user=Depends(get_current_user)):
    """
    Controller: Apenas encaminha o usuário autenticado para o Service.
    Atende Requisito 4.8. 
    """
...
def export_user_data(current_user=Depends(get_current_user)):
    """
    Controller: Solicita a exportação de dados ao Service.
    Atende Requisito 4.9. 
    """
...
def delete_account(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """
    Controller: Recebe o comando de exclusão e delega ao Service.
    Atende Requisito 4.10. 
    """
